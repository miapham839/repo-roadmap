-- TiDB Schema for Repo Roadmap Agent
...
CREATE TABLE IF NOT EXISTS user_profiles (
  ...
  preferences JSON, -- { prefer: 'code'|'docs'|'tests', stack: ['react', 'node', ...] }
  profile_text TEXT, -- concatenated for embedding
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS embeddings (
  ...
  vector VECTOR(1536) NOT NULL, -- OpenAI ada-002 or text-embedding-3-small dimension
  snippet TEXT, -- first 500 chars for display
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  INDEX idx_entity (entity_type, entity_id),
  VECTOR INDEX idx_vector (vector)
);
...
CREATE TABLE IF NOT EXISTS roadmaps (
  ...
  user_id VARCHAR(255) NOT NULL,
  repo_id INT NOT NULL,
  summary TEXT, -- LLM-generated project structure summary
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
  FOREIGN KEY (repo_id) REFERENCES repos(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS roadmap_steps (
  ...
  title VARCHAR(500) NOT NULL,
  details TEXT,
  linked_issue_id INT, -- nullable, references issues(id)
...
CREATE TABLE IF NOT EXISTS actions (
  ...
  user_id VARCHAR(255) NOT NULL,
  roadmap_id INT NOT NULL,
  step_id INT, -- nullable, links to roadmap_steps
  type ENUM('calendar', 'notion') NOT NULL,
  payload JSON, -- event details, external IDs, etc.
  status ENUM('pending', 'created', 'failed') DEFAULT 'pending',
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
  FOREIGN KEY (roadmap_id) REFERENCES roadmaps(id) ON DELETE CASCADE,
  FOREIGN KEY (step_id) REFERENCES roadmap_steps(id) ON DELETE SET NULL,
